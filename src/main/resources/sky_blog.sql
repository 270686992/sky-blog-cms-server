...
CREATE TABLE IF NOT EXISTS `category` (
  ...
  `image`          VARCHAR(255) DEFAULT NULL COMMENT '文章分类图标 url',
...
CREATE TABLE IF NOT EXISTS `article` (
  ...
  `image`          VARCHAR(255) DEFAULT NULL COMMENT '文章封面图 url',
  ...
  `comment_num`    INT(10) UNSIGNED NOT NULL DEFAULT '0' COMMENT '文章评论数量',
...
CREATE TABLE IF NOT EXISTS `friend_link` (
  ...
  `image`          VARCHAR(255) DEFAULT NULL COMMENT '友链图标 url',
...
CREATE TABLE IF NOT EXISTS `web_site_info` (
  ...
  `avatar`                  VARCHAR(255) NOT NULL COMMENT '站点作者头像 url',
  `icon`                    VARCHAR(255) NOT NULL COMMENT '站点图标 url',
  `commentator_image`       VARCHAR(255) NOT NULL COMMENT '评论者/留言者默认头像 url,用户没有设置头像时使用默认头像',
