...
CREATE TABLE IF NOT EXISTS `web_site_info` (
  ...
  `application_explain`     VARCHAR(1000) DEFAULT NULL COMMENT '友链申请说明',
  `about_me_explain`        VARCHAR(1000) DEFAULT NULL COMMENT '关于我的说明',
  `about_site_explain`      VARCHAR(1000) DEFAULT NULL COMMENT '关于站点的说明',
  `about_copyright_explain` VARCHAR(1000) DEFAULT NULL COMMENT '关于版权的说明',
  `special_explain`         VARCHAR(1000) DEFAULT NULL COMMENT '特别说明',
  `announcement`            VARCHAR(1000) DEFAULT NULL COMMENT '站点公告',
  `create_time`             DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `update_time`             DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  `delete_time`             DATETIME(3) DEFAULT NULL COMMENT '删除时间,软删除',
  PRIMARY KEY (`id`)
