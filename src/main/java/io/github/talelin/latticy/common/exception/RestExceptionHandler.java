...
public class RestExceptionHandler {
    ...
    /**
     * HttpMessageConversionException 自己加的
     */
    @ExceptionHandler({HttpMessageConversionException.class})
    public UnifyResponseVO processException(HttpMessageConversionException exception, HttpServletRequest request, HttpServletResponse response) {
        log.error("", exception);
        UnifyResponseVO result = new UnifyResponseVO();
        result.setRequest(getSimpleRequest(request));
        String errorMessage = CodeMessageConfiguration.getMessage(10170);
